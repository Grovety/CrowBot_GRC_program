...
// --------- Racing tuning API ----------
struct RaceParams {
  float max_speed_scale = 1.0f;    // total speed multiplier (1.0 = default)
  float turn_scale = 1.0f;         // multiplier for rotation (1.0 = default)
  float accel_coeff = 1.0f;        // acceleration coefficient (new variable, 1.0 = default)
};
RaceParams race; // current parameters
...
// A slightly handy clamp for numbers
template<typename T>
T clampVal(T v, T lo, T hi) {
  return (v < lo) ? lo : (v > hi ? hi : v);
}

// Default initialization (default values)
...
// motor ramp taking into account accel_coeff
...
const int RAMP_INTERVAL_MS = 20;  // ramp update interval

// Set the target values of the motors (with ramping)
...
// Updating ramping motors (to be called in a loop or task)
void updateMotorRamp() {
  unsigned long now = millis();
  if (now - last_ramp_ts >= RAMP_INTERVAL_MS) {
    float step = race.accel_coeff * (MOTOR_MAX_VALUE / 10.0f);  // the step depends on accel_coeff
...
void adjustRaceFloat(float &field, float delta, float lo, float hi, const char *name) {
  ...
  Serial.printf("%s -> %.3f\n", name, field);
  // TODO: persist to Preferences if necessary
}
  ...
  char msg[MAX_GRC_MSG_LEN];
    ...
    if (!imu_control_mode) {
      if (value == "GO FORWARD" || value == "xiang qian zou")  //forward
      {
        ...
        setTargetMotor(int(160.0f * globalSpeed), 0, int(160.0f * globalSpeed), 0);  // Using ramps
        delay(600);
      ...
      if (recv_imu_coords) {
        ...
        float globalSpeed = race.max_speed_scale * speed;  // Scaling the speed
// After calculating l and r from poly_transform
l *= globalSpeed;
r *= globalSpeed;

// Add the effect of turn_scale on the difference between motors
if (abs(l - r) > 0.1f) {  // if there is a turn
...
setTargetMotor(lf, lb, rf, rb);  // Using ramps
...
const uint32_t BUZZER_FREQ = 5000;  // base PWM frequency, 5 kHz
const uint8_t  BUZZER_RES  = 8;    // 8‑bit resolution

if (!ledcAttach(BUZZER_PIN, BUZZER_FREQ, BUZZER_RES)) {
  Serial.println("Error: failed to set up LEDC for pin 33");
}
  //Patrol
  pinMode(QTI_L, INPUT);
  pinMode(QTI_R, INPUT);
  ...
  // Setting each pin to 255 Hz, 8 bits
  if (!ledcAttach(pin, 255, 8)) {
    Serial.printf("Error configuring LEDC on the pin %d\n", pin);
    // you can add error handling, for example, an infinite loop
  }
}
  //infrared remote control
  irrecv.enableIRIn();
  ...
  if (xMusicQueue == NULL) {
    Serial.println("Error: failed to create xMusicQueue");
  } else {
  ...
  initRaceDefaults();  // Initialization of default values
  xTaskCreate(ramp_task, "ramp_task", 1024, NULL, 1, NULL);
}


//while（1）
void loop() {
  updateMotorRamp();  // We update the ramp motors every cycle
